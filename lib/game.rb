...
class Game
  ...
  ## GAME BEGINS
      ...
      puts "* * User wins, computer defeated! * *" if @cpu_hit_count == @cpu_ships.map{|ship| ship.health}.sum && @user_hit_count != @user_ships.map{|ship| ship.health}.sum 
      puts "* * Game Over! Computer wins! * *" if @user_hit_count == @user_ships.map{|ship| ship.health}.sum && @cpu_hit_count != @cpu_ships.map{|ship| ship.health}.sum
      puts "* * Tie! We both win! * *" if @user_hit_count == @user_ships.map{|ship| ship.health}.sum && @cpu_hit_count == @cpu_ships.map{|ship| ship.health}.sum
      puts "\n"
